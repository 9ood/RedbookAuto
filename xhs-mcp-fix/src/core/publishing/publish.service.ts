...
export class PublishService extends BaseService {
  ...
  async publishNote(
    ...
    try {
      ...
      try {
        ...
        // Submit the note
        await this.submitPost(page, saveAsDraft);
  ...
  private async submitPost(page: Page, saveAsDraft: boolean = false): Promise<void> {
    const submitSelectors = saveAsDraft
      ? [
          'button.d-button.white',
          'button.d-button-default.white',
          'button.custom-button.white',
        ]
      : [
          'button.d-button.bg-red',
          'button.d-button-default.bg-red',
    ...
    if (!submitButton) {
      logger.warn('Could not find submit button automatically. Waiting 5 minutes for manual operation...');
      logger.warn('Please manually click "发布" or "暂存离开" button in the browser window.');
      await sleep(300000);
    ...
    try {
      ...
      logger.warn(`Failed to click submit button automatically. Waiting 5 minutes for manual operation...`);
      logger.warn('Please manually click "发布" or "暂存离开" button in the browser window.');
      await sleep(300000);
  ...
  async publishVideo(
    ...
    try {
      ...
      try {
        const noteId = await this.executeVideoPublishWorkflow(page, title, content, resolvedVideoPath, tags, saveAsDraft);
